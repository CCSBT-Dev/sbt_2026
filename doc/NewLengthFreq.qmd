...
assessment_length_freq <- read_csv(here("OMMP16", "csv_2026", "length_freq.csv"), show_col_types = FALSE)
latest_assessment_year <- max(assessment_length_freq$Year, na.rm = TRUE)
```

The Japanese size data currently loaded from `r size_file` run through `r max(df1$year, na.rm = TRUE)`. The GAM CPUE data currently loaded from `r model_file` run through `r max(df2$year, na.rm = TRUE)`, so the joined GAM-weighted estimates run through `r latest_data_year`. The assessment length-frequency data loaded from `r here("OMMP16", "csv_2026", "length_freq.csv")` run through `r latest_assessment_year`.
...
```{r join}
dfj <- df1.1 |> inner_join(df2, by = join_by(year, month, lat5, lon5))
```
...
ll1_n_by_year <- assessment_length_freq |>
  filter(Fishery == 1) |>
  distinct(Year, N)

lf_gam_sbt_fishery <- lf_gam_sbt_wide |>
  rename(Year = year) |>
  mutate(Fishery = 1L, .before = Year) |>
  left_join(ll1_n_by_year, by = "Year") |>
  relocate(N, .after = Year)

missing_bins <- setdiff(sbt_length_bins, names(lf_gam_sbt_fishery))
lf_gam_sbt_fishery[missing_bins] <- 0
lf_gam_sbt_fishery <- lf_gam_sbt_fishery |>
  select(Fishery, Year, N, all_of(sbt_length_bins)) |>
  arrange(Fishery, Year)

write_csv(lf_gam_sbt_fishery, here("doc", "lf_gam_cpue_sbt_fishery_length_freq.csv"))
# lf_gam_wide
...
#write_csv(assessment_length_freq, "lf_assessment.csv")
flen=80;llen=190
library(ggridges)
...
The Figure 2 GAM CPUE length-frequency matrix is written in SBT fishery length-composition format as [`lf_gam_cpue_sbt_fishery_length_freq.csv`](lf_gam_cpue_sbt_fishery_length_freq.csv).
